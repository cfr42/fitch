...
\date{Version 1.1\\ September 7, 2026}
...
preamble of your {\LaTeX} file. \textbf{(If your document uses the |nd|
environment in math mode, you should specify |arrayenv=array| instead.
Using |nd| in math mode is no longer recommended.)}
...
However, PDF tagging is incompatible with the |array|
environment used in prior versions to typeset derivations. For
compatibility with PDF tagging, use the |tabular| environment instead,
by loading the package with the option |arrayenv=tabular|. This will
result in errors if you use the |nd| environment in math mode, so it
is recommended to use the |fitchproof| environment instead. See
Section~\ref{sec-usage} and \ref{sec-tagging} for details.
...
|fitch| with the |arrayenv=tabular| option. This will become the
default in a future release. By default, proofs are set inside a
|flushleft| environment (with |\topsep| spacing). The environment used
can be changed using the |proofenv| option (see
Section~\ref{sec-customization}).
...
\LaTeX{} environment. By default, |nd| uses the |array| environment to
do this, so the |nd| environment must must be used in math mode, i.e.,
it should be surrounded by |$...$| or |\[...\]|, unless a different
environment is specified using the |arrayenv| option. This used to be
the default and recommended way of using |fitch|.

\NewIn{1.1} In order to produce accessible PDFs it is
no longer recommended to use the |nd| environment in math mode with 
the default |arravenv=array|. Instead, use the |fitchproof| environment (or
...
Formuals in derivations are converted to MathML if the document is set
up for that. Numerical line labels in derivations and line number
references are \emph{not} converted to MathML. The tables resulting
from |fitchproof| and |nd| environments are tagged as presentation
tables (since they don't and can't have table headers, which would
otherwise be required for PDF/UA-2 compliance).
